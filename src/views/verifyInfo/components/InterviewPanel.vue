        ...
        <div class="reference-card" v-if="showAnswer && currentQuestion && !isVerified">
          <div class="reference-section">
            ...
            <div class="reference-content">
              <p v-for="(p, i) in formattedAnswer" :key="i">{{ p }}</p>
            </div>
          </div>
          ...
          <div class="points-section">
            ...
            <div class="points-list">
              <div class="point-item" v-for="(point, i) in currentQuestion.keyPoints" :key="i">
...
import { getRandomQuestions, getQuestionsByCategory, loadQuestions } from '../utils/questionBank';
...
export default {
  ...
  created() {
    // 初始化题目总数
    this.totalQuestionCount = loadQuestions().length;
    // 默认选择第一个选项（10%）
  ...
  computed: {
    ...
    formattedAnswer() {
      if (!this.currentQuestion?.referenceAnswer) return [];
      return this.currentQuestion.referenceAnswer.split('\n').filter(p => p.trim());
    }
  },
  methods: {
    handleStart() {
